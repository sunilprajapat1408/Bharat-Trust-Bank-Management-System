...
public class SignupStep2 extends JFrame implements ActionListener {
    ...
    public void actionPerformed(ActionEvent e) {
        ...
        try {
            if (InputValidator.isEmpty(textPan.getText()) || InputValidator.isEmpty(textAadhar.getText())) {
                JOptionPane.showMessageDialog(null, "Fill all the fields");
            } else {
                User user = new User();
                user.setFormNumber(formno);
                user.setReligion(rel);
                user.setCategory(cate);
                user.setIncome(inc);
                user.setEducation(edu);
                user.setOccupation(occ);
                user.setPanNumber(pan);
                user.setAadharNumber(addhar);
                user.setSeniorCitizen(scitizen);
                user.setExistingAccount(eAccount);
                
                userService.registerUserAdditionalDetails(user);
                new SignupStep3(formno);
                setVisible(false);
            }
